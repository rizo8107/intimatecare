...
const PaymentTestPage: React.FC = () => {
  ...
  const handleCheckStatus = async () => {
    if (!storedCfOrderId) { // Use storedCfOrderId instead of paymentOutcome.order_id
      toast({
        title: 'Error',
        description: 'Cashfree Order ID not found to check status.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      const response = await fetch(verifyWebhookUrl, {
        ...
        body: JSON.stringify({ order_id: storedCfOrderId }),
      });
